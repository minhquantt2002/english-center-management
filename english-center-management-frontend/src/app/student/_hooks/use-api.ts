...
export const useStudentApi = () => {
  ...
  const getClassMaterials = useCallback(async (classId: string) => {
    ...
    setError(null);
    try {
      const response = await api.get(`/student/classes/${classId}/materials`);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Có lỗi xảy ra';
      setError(errorMessage);
  ...
  const submitAssignment = useCallback(
    async (classId: string, assignmentId: string, submissionData: any) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const updateStudentProfile = useCallback(async (profileData: any) => {
    setLoading(true);
    setError(null);
    try {
  ...
  // Get class schedules
  const getClassSchedules = useCallback(async (classId: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  // Get class scores
  const getClassScores = useCallback(async (classId: string) => {
    setLoading(true);
    setError(null);
    try {
