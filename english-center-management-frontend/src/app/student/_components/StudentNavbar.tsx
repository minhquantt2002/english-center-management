...
const StudentNavbar: React.FC<StudentNavbarProps> = ({ onToggleSidebar }) => {
  ...
  const generateBreadcrumbs = () => {
    ...
    if (paths.length === 1 && paths[0] === 'student') {
      return [{ name: 'Tổng quan', href: '/student' }];
    }

    breadcrumbs.push({ name: 'Tổng quan', href: '/student' });

    if (paths.length > 1) {
      const pageName = paths[1];
      const pageNames: { [key: string]: string } = {
        'studying-schedule': 'Lịch học',
      ...
      breadcrumbs.push({
        ...
        href: `/student/${pageName}`,
      });
    }

    // Handle personal-info which is outside student folder
    if (pathname === '/personal-info') {
      return [
        { name: 'Tổng quan', href: '/student' },
  ...
  const notifications = [
    {
      ...
      message: 'Bài tập Writing số 5 đã có sẵn',
      time: '10 phút trước',
      unread: true,
    },
    {
      id: 2,
      title: 'Điểm kiểm tra có sẵn',
      message: 'Kết quả bài kiểm tra Nghe đã sẵn sàng để xem',
      time: '2 giờ trước',
      unread: true,
    },
    {
      ...
      message: 'Lớp Speaking nâng cao bắt đầu sau 30 phút',
      time: '4 giờ trước',
      unread: false,
    },
  ];
      ...
      <div className='h-full flex items-center justify-between px-6'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div className='relative hidden md:block'>
            ...
            <input
              type='text'
              placeholder='Tìm kiếm bài học, bài kiểm tra...'
              className='block w-64 pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500'
            />
          </div>
          ...
          <div className='relative'>
            ...
            >
              <img
                src='https://images.unsplash.com/photo-1494790108755-2616b612b3fd?w=150&h=150&fit=crop&crop=face'
                alt='Student'
